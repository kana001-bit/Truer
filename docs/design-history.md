...
2026-07-16 で apply の書き先を「Truer 所有の補正済み DXF」に決めたとき、暗黙に想定していた用途は「裁つ前にこの 1 着を直す」だった。
だがその後、もう一つの用途——**制作の途中で Valentina のパラメトリック編集を続けながら直したい**——に正面から向き合うと、あの決着だけでは足りないと分かった。
Valentina は `.val` しか読めず、DXF は書き出せるだけで読み戻せない。
つまり Truer が焼いた補正済み DXF は、裁断には使えても Valentina へ戻して編集を続ける道が無い。途中工程には行き止まりだった。

素直な解決策は、また「Truer が `.val` を書けばいい」に戻る。今回はそれを前より深く理解したうえで棄てた。
`.val`はパラメトリックな構築レシピ（式と、他点から導出される点）で、DXF はそれを評価して平らにした結果である。
**平面（DXF）→パラメトリック（.val）の向きは本質的に一方通行**——根っこは「**Truer は Valentina のモデル評価を再現できない**」の一点だ。
だから DXF→`.val` に構築を復元することもできないし、`.val` に式を書いて補正しても「補正後の線」を描く（＝モデルを評価する）のは Valentina にしかできず、Truer 単独では正しい preview を出せない＝ **preview==apply** が破れる（この preview の破れは 2026-07-16 で `.val` を棄てた理由の一つで、一方通行はその根っこにある形だと今回分かった）。
dxf→val の変換器を挟む案も、DXF に補助線が無い以上「元の`.val` を人が編集して Truer の形に合わせる」ことになり、Truer の出力が実質使われない＝Truer が要らなくなる。
CAD を DXF ネイティブ（LibreCAD 等）へ替えれば round-trip の壁は消えるが、それは Valentina のパラメトリック能力（寸法駆動・グレーディング）を捨てるということで、それは捨てられない。
どの道も、`.val` を Truer が書く方向は塞がっていた。

### そもそも Valentina 自身が、長さ違いを教えてくれないのはなぜか
.val を棄てたあと、もっと素朴な問いが引っかかった。
そもそも Valentina 自身が、長さ違いを教えてくれないのはなぜ？
Valentina は式で点と線を引いているのだから、長さは計算できているはずだ。
使いこなせていないせいで検出機能を見落としているだけではないか、と疑って調べた。

分かったのは、そうではないということだった。
Valentina は seam 長を揃える解決策を持たず、二辺は独立に引かれるから普通にズレる。
そして measured Δ が「いせ込みの意図」か「ただのミス」かは、Seamlint や Truer と同じく Valentina にも計測だけでは判別できない。
Valentina もまた、何も判断せず、何も言わない道具だった。

この符合がいちばんの発見になった。Valentina に足りないのは検出ではなく宣言だ。
ズレを防ぐ手段は Valentina の構築側にある（二辺の長さをリンクすれば等長が保証される）——が、Valentina はそれを強制も警告もしない。
だから「防ぐのは構築、判断するのは人の宣言、捕まえるのは linter」という切り分けが、ここで決まった。

後で掘ると、この "言わない" は徹底していた。
Valentina が警告するのは式のパースエラーや交点なしのような「計算が**成立しない**」系だけで、リンクで landmark がずれる**寸法の過拘束は警告しない**——成立してしまうからだ。
一方向の DAG 評価器で各点は定義を一つしか持てず、不変条件を一つも持たない（＝ solver ではない）から、「拘束が多すぎる」という状態を**表現しようがない**。
だから過拘束の検知は Valentina には置けず、宣言済みの不変条件を握る **linter の再チェックループ**でしか捕まらない——これは後段の V5 の構造的な裏付けであり、「なぜこのツールチェーンが要るのか」の一番きれいな説明にもなった。

### 決着は、「正典（authoritative）」と「出力」を切り離すことだった。
**縫うものは常に val 由来の幾何を正典とする**——val を人が直し、そこから再エクスポートした DXF を裁つ。
縫い代・ノッチ・地の目は Valentina が自動再計算するので、これが一番安全だ。
**Truer が焼く補正済み DXF は「val を（Valentina の構築が過拘束で）その場では直せないが、この 1 着は裁ちたいときの緊急補正」に格下げ**し、本番裁断ファイル（＝val 由来）には据えない。
そして途中工程の本筋は、`.val` を書くことではなく **提案として出すこと**にした——Truer は「どの辺を、どれだけ、どう直すか（目標と種類）」を計算して人へ渡し、人が Valentina で当てる。
「val=正典 / 焼き DXF=緊急」は、そのまま「①治す / ②凌ぐ」の軸になった。

この符合から、もう一つの invariant が表に出た——**意図は推論せず宣言する**。どのツールも measured Δ の意味（ease かミスか）を計測だけでは決められないからだ。
だから賢く推論させるのではなく、**人が connector に ease を宣言する**（既定 0＝揃うべき）。
Seamlint は measured Δ を宣言値と突き合わせ、Truer の reconcile 目標も宣言 ease になる。
equal は ease=0 の特殊ケースにすぎない。
これは「対応（どの辺が縫い合うか）は人が part–part で宣言する」既存の思想の延長で、ease の threading も既存の notch 署名と同じ経路を通せる。

修正そのものは二層に分けた。
**①構造リンク**——Valentina の構築で二辺の長さを共有（`VLineLength`/`VIncrement`で配線）すれば、等長が構築で保証され再発しない。
これが mid-process の本命で、既定に推す。**②corner-slide**——共有コーナーを隣辺の直線上に沿って滑らせて長さを合わせる幾何パッチ。
Seamlint の `structuralEdges`（角＝各辺の端点、隣辺＝ループ順で k±1、finished 長）を読めば **Seamlint を改修せずに解ける**ことは確認した。
だが②は本質的に band-aid で、val が未リンクのままなら再生成で mismatch が戻る。
だから corner-slide は「機構」から「その場しのぎの fallback」へ格下げした。
そして **Truer は①と②のどちらが正しいかを決められない**——①が可能か（過拘束にならないか）は Valentina の構築 DAG 次第で、Truer はそれを読まない。
だから断定せず、「まず①リンクを試せ、過拘束なら②で低カップリングの角に Δ を吸わせよ」という決定木を人へ渡す。
**Truer は「決める頭脳」であって「書く手」にはならない。** 
こうすれば Valentina を再現せずに済み、preview==apply も割れない（Truer が描く/書くのは常に自分で計算した幾何——advisory な目標長と、任意の緊急 DXF——に限られる）。
Truer が構築に対して盲目であること自体が、木を出す理由になっている。

残る懸念も正直に書いておく。**共有コーナーを動かすと、その角を端点に持つ隣辺も道連れで変わる**（V1・最重要）。
角が Valentina の導出点なら座標では直接動かせないので、指示は必ず「この辺の finished 長を X にせよ」の目標長で出す（V2）。
角を滑らせると notch 位置がずれ、Seamlint のペア同定（notch 署名）が揺れうる（V3、記録済みedgeDigest でドリフトを明示検出する）。
縫い代は val 側では自動再計算なので非懸念（V4——緊急 DXF 直裁ちのときだけ壊れる＝②を band-aid と割り切る裏付け）。
辺ごとに直すと A↔B が振動しうるが、それは Truer の失敗ではなく「全seam 長を同時に満たせない＝型紙が過拘束」という本物の所見で、判定は connector グラフを持つ Loomit の領分に置くのが筋だ（V5）。
これらは完全には防げない。
Truer は「①低カップリングの角で吸い、②目標長で指示し、③連動を警告して、再エクスポート→Seamlint 再チェックで捕まえる」の三点で受け、防止でなく事後捕捉に倒す。
ここでも「わからないもの・見えないものは、直すふりをせず人に返す」という Truer の背骨は変わっていない。
...
- **seam_length の実補正（B）。** 形は上の 2026-07-18 章で握った（出力＝val 正典 / 焼き DXF＝緊急、修正＝
  ①構造リンク / ②corner-slide の二層、意図＝宣言 ease、指示は advisory）。残るのは実装と cross-repo:
